...
import { GetCommisionDetails } from "../../../Api/Payout";
...
const CommisionDetails = () => {
  ...
  const FetchTransaction = async () => {
    try {
      setLoading(true);
      let res = await GetCommisionDetails();
      if (res.status) {
  ...
  const columns = useMemo(
    () => [
      ...
      { Header: "SPONSER NAME", accessor: "Sponser.Name" },
      { Header: "SPONSER ID", accessor: "Sponser.referralCode" },
      { Header: "SPONSER RANK", accessor: "Sponser.Rank" },
      {
        Header: "MONTH",
        accessor: "month",
  ...
  const totalAmount = useMemo(() => {
    return page
      .reduce(
        (sum, { original }) => sum + (parseFloat(original.commision) || 0),
        0
      )
      .toFixed(2);
  }, [page]);
