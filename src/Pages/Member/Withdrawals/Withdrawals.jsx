...
import { GetWalletTransaction, UpdateWithdrawl } from "../../../Api/Wallet";
...
const Withdrawals = () => {
  ...
  const fetchTransaction = async () => {
    try {
      ...
      let res = await GetAllTransaction(query);
      ...
      { Header: "STATUS", accessor: "status" },
      {
        ...
        Cell: ({ row }) => (
          <div className="d-flex justify-content-center">
            <FaEdit
              onClick={() => {
                HandleTransactionAction(row.original._id);
              }}
              style={{
                cursor: "pointer",
                margin: "0 10px",
  ...
  const HandleTransactionAction = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // Approve
        await processTransaction(id, "Approve", "Approved Successfully");
        ...
        }).then(async (remarksResult) => {
          if (remarksResult.isConfirmed) {
            await processTransaction(id, "Reject", remarksResult.value);
          }
        });
      } else {
        // Cancel action
  ...
  const processTransaction = async (id, actionType, remarks) => {
    setApiloader(true);
    try {
      let res = await UpdateWithdrawl(
        {
          status: actionType === "Approve" ? "Success" : "Rejected",
          message: remarks,
        },
        id
      );
      ...
      if (res.status) {
        Swal.fire(
          `${actionType} Success!`,
          `Transaction has been ${actionType.toLowerCase()}ed.`,
          "success"
        );
